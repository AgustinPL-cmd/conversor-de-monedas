# Conversor de Monedas en Tiempo Real

<p align="center">
  <a href="https://imgbb.com/"><img src="https://i.ibb.co/R688PRy/Conversor-de-MONEDAS.gif" alt="Conversor-de-MONEDAS" border="0"></a>
</p>

## 📌 Descripción

Este proyecto es un conversor de monedas en tiempo real que utiliza la API de ExchangeRate-API.  
Con esta aplicación, puedes convertir fácilmente entre diferentes monedas utilizando las tasas de cambio más recientes disponibles.

## 🚀 Características

- **Conversión en Tiempo Real**: Obtén tasas de cambio actualizadas en tiempo real.
- **Interfaz Intuitiva**: Una interfaz de usuario fácil de usar y atractiva.
- **Multiples Monedas**: Soporte para múltiples monedas globales.
- **Historial de Conversión**: Guarda y visualiza conversiones pasadas.

## 📸 Capturas de Pantalla

<a href="https://ibb.co/pdHVnNL"><img width = 100% src="https://i.ibb.co/x6n93w7/image.png" alt="image" border="0"></a>
*PANTALL PRINCIPAL*
<br>
<br>

<a href="https://ibb.co/wdjZ8Qc"><img width = 100% src="https://i.ibb.co/XWMf65Y/image.png" alt="image" border="0"></a>
*Elige la moneda a convertir y la cantidad*
<br>
<br>

<a href="https://ibb.co/dG6DrXT"><img width = 100% src="https://i.ibb.co/Ln618BF/image.png" alt="image" border="0"></a>
*Elige la moneda a la cual convertir*
<br>
<br>

<a href="https://imgbb.com/"><img  widht = 100% src="https://i.ibb.co/0y9fwk8/image.png" alt="image" border="0"></a>
*RESULTADO de la conversión*
<br>
<br>

<a href="https://ibb.co/Kbtbd6J"><img width = 100% src="https://i.ibb.co/hLGLbsS/image.png" alt="image" border="0"></a>
*Sale del programa y se muestra el Historial y la hora en la que se hizo la conversión*
<br>
<br>

## 🎥 Video Demo

<a href="https://ibb.co/T2Hwpyv"><img widht = 100% src="https://i.ibb.co/c3NyMqg/Dise-o-sin-t-tulo.gif" alt="Dise-o-sin-t-tulo" border="0"></a>
*Haz clic para ver el vídeo*

## 🛠️ Tecnologías Utilizadas

- **Backend**:
  - JAVA
  - Librería Gson

- **API**:
  - ExchangeRate-API

## 📦 Instalación

1. Clona el repositorio:
   ```sh
   https://github.com/AgustinPL-cmd/conversor-de-monedas

